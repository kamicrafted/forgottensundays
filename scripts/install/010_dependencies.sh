...
usermod -u "$CM_USER" www-data
usermod -G "$CM_GROUP" www-data
...
apt-get update
apt-get install -y --no-install-recommends           libfreetype6-dev         libjpeg62-turbo-dev         libmcrypt-dev         libpng12-dev         libmagickwand-dev         vim curl git wget unzip
